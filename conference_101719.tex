...
\begin{algorithm}[!h]
  ...
  \Function{Lock}{$T,K, M, V$}
    ...
    \ForAll {such ${l \in lock\_table}$, \\ ${VIOLATABLE(K,T.xid,l,M,V) }$ is \textbf{True}}
        ...
        \State {${T.vlocks \gets T.vlocks \cup l}$}
        \If {${T.xid \notin s.out }$}
...
\begin{algorithm}[!h]
  ...
  \Function{Prepare}{$T$}
    ...
    \State \hskip1.5em \textbf{if} {${T.violate = EARLY}$ \textbf{and} wait timeout  
    \newline ${\exists l \in T.vlocks}$, ${ T.xid < l.xid }$}
    \State \hskip3em  write prepare abort log
    \State \hskip3em response TM message \emph{\{Prepare Abort\}} \Comment{die}
    \State \hskip3em \textbf{return}
    \State \hskip1.5em \textbf{end if}
    \State \textbf{end wait}
    \If {${T.in < 0}$}
