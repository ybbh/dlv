...
%Optimistic CC scales well when there is little contention but suffers a high abort rate when dealing with a workload of high degrees of contention.
...
%We focus on distributed DBMS which uses locking scheme and coordination protocol on a replicated layer, especially those running transaction processing on   geo-replicated layer.
...
%These two types of transaction models employ different message flow.
...
%${T_j}$ has a commit dependency on ${T_i}$ and  ${T_i}$ has not been committed.
...
%When we extended the non-strict locking scheme to distributed transactions, transaction dependency maintenance is more complex.
...
%For recoverability, we must ensure that ${T_j}$ cannot commit if ${T_i}$ has not been committed.
...
% and a CLV approach customized for the geo-replicated scenario.%optimize both violate at the 1st phase(CLV1P) and 2nd phase(CLV2P).
...
\caption{throughput and abort rate of different possible hot items}
...
\caption{throughput and abort rate when adding possible hot item row, in gathered mode}
