...
We used the YCSB workload, in which we set the value of \emph{theta}- to 0.2 and the number of concurrent clients connected to each shard to 100.
...
As the late vioilation scheduler schedulers adopted the wait-die policy, transactions normally chose to abort when confronted with serious contention.
Therefore, the abort rates measure how the performance was affected by contention.
The result shows that DLV1x and DLV1 could deal with contention better than the others.
As they violate locks earlier than S2PL, they are subject to less contention than S2PL. (DLV1x releases locks earlier than DLV2.)
...
\begin{figure}[tbp]
  ...
  \captionsetup[subfigure]{margin={0cm,0cm}}
  \subfloat[]
      { \includegraphics[scale=0.4] {figure/plot_ycsb_add_LogForceT_LogForceT_TPM_gather} \label{fig:ycsb_add_log_cost:tpm}}

  \captionsetup[subfigure]{margin={0cm,0cm}}
  \subfloat[]
      { \includegraphics[scale=0.4]{figure/plot_ycsb_add_LogForceT_LogForceT_Abort_gather} \label{fig:ycsb_add_log_cost:abort}}

\caption{throughput and abort rate of
different terminal different log write costs, using YCSB workload}
...
DLV0 and DLV1 do improve throughput when it has a little abort rate than S2PL.
Among the DLVs, DLV0 and DLV1 perform well on medium contention. 
...
The maximum throughput S2PL could achieve around 41,000 TPM (Transactions per Minute),
while the throughput of DLV0 and DLV1x could achieve 70,000 TPM.
...
\caption{throughput and abort rate when adding terminal number of each shard,
(p) label stands the scheduler use wait-die for deadlock prevention, and (b) label means the scheduler use deadlock detection. 
}
...
In this experiment, we apply wait-die for all the schedulers to prevention deadlock. 
