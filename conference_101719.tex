...
In this paper, we show that the direct application of speculation is often unhelpful in a distributed environment.
...
It may fail to scale when confronted with the highly contended workload.
...
as the commit critical path will involve many message round trips.
...
Many reasons can cause a transaction to abort. In general, they can be categorized as:
...
%When we extended the non-strict locking scheme to the distributed transaction, transaction dependency maintaining is more complex.
...
%The scheduler needs to maintain commit dependencies to guarantee serializability and recoverability when exploiting non-strictness.
...
Only DLV1x  needs an additional message round trip to prevent the violation from occurring before deterministic aborts.
...
The data were partitioned by into 4 shards. Each shard had 3 replicas spread across 3 AZs, 
...
Geo-replication has a big impact on performance. The higher the replication latency, the lower the throughput of transaction processing.
...
In the experiments on YCSB, we fixed the number of clients of every shard to 200 and varied the parameter \emph{theta} (the skewness of the Zipfian distribution) to adjust the degree of contention.
...
The results also show that contention had a big impact on performance.
...
Moreover, we found that not all lock violation approaches can be beneficial. We identified the right time for the violation to maximize its effects in distributed transactions. 

A distributed transaction remains a challenge to our community. Recent progress of the HA technologies has successfully made distributed transactions less vulnerable to failures.
...
While lock violation is an effective technique to optimize the performance, it is far from the ultimate solution.
...
Nevertheless, considering the ever-increasing scale of modern Web applications, the distributed transaction is definitely a direction worth further investigation.
