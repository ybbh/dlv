...
This vast difference of RTT let use consider exploit  
the speculative technique to reduce lock blocking.
\end{highlighted}
...
1. 
To the best of our knowledge, this is the first study of lock violation on GDDB.
...
\begin{highlighted}
 ...
 In this sense, CLV is more related our work.

 Although CLV can be extended to work with distributed systems, 
 how to use lock violation to empower distributed transactions still remains as a challenge.
...
%We can see that a DP transaction costs more message round trips than an NDP transaction.
%In a DP transaction, the $TM$ must explicitly notify the $RM$s to prepare to commit.
...
%In the next section, we will introduce our design to solve the recoverability problem.
