...
\begin{algorithm}[!h]
  \caption{Lock a key ${T}$. Read and write a key}
  \begin{algorithmic}[1]
  \Function{Lock}{$T,K, M, V$}
    \State {\textbf{wait if} ${\exists}$ such ${l \in lock\_table }$,
      \newline $CONFLICT(K, T.xid, l, L)$ \textbf{and}  
      \newline \textbf{not} $ VIOLATABLE(K, T.xid, l, M, V)$}
      \State \hskip2em \textbf{if}  {${T.xid > l.xid } $ \textbf{and} wait timeout}
          \State \hskip4em  \textbf{return} nil \Comment{lock die error}
      \State \hskip2em \textbf{end if} 
    \State \textbf{end wait}

    \ForAll {such ${l \in lock\_table}$, \\ ${VIOLATABLE(K,T.xid,l,M,V) }$ is \textbf{True}}
        \State {${s \gets}$  transaction context retrieved by ${l.xid}$}
        \If {${V == EARLY}$ \textbf{or} $($ ${V = LATE}$ \textbf{and} \newline 
        ${l.lmode = WRITE}$ \textbf{and} ${L = READ}$ $)$}
        \If {${T.xid \notin s.out }$}
        ...
        \EndIf
    \EndFor
    \State {create lock ${l}$ if ${l' \notin lock\_table} $, \newline ${l'.xid \gets T.xid}$,  ${l'.key \gets K, l'.mode \gets M, l'.violate \gets V}$}
  ...
  \caption{Lock a key ${T}$. Read and write a key}
  \begin{algorithmic}[1]
  \Function{VIOLATABLE}{$T,K, L, V$}
  \If {$CONFLICT(T, l, K, M)$ \textbf{and}\\ ${l.violate \neq NONE}$}
  \If{${V=EARLY}$}
    \State \textbf{return} ${T.xid > l.xid}$
  \ElsIf{${V=LATE}$}
    \State \textbf{return} ${l.violate = LATE}$
  \EndIf
\EndIf
\State \textbf{return} \textbf{False}
  \EndFunction
  \label{func:violatable}
  \end{algorithmic}

  \caption{transaction T lock a key ${T}$ in ${M}$ mode, use ${V}$ violation}
  \begin{algorithmic}[1]
  \Function{Conflict}{$T, L, K, M$}
    \If {${T.xid \neq l.xid}$ and ${K = l.key}$}
...
\begin{algorithm}[!h]
  ...
  \Function{Read}{$T, K$}
    ...
    \Else
      ...
      \State {${v \gets}$  retrieve value of ${K}$ from the newest version}
...
\begin{algorithm}[!h]
  ...
  \Function{Commit}{$T$}
    ...
    \For {${t \in T.out}$}
      ...
      \If {${t.in = 0}$}
        \State report ${t.in = 0}$ \Comment{stop waiting on this transaction, in line 2 of function PREPARE}
      \EndIf
    \EndFor
...
\begin{figure}[htbp]
  ...
  \captionsetup[subfigure]{margin={0cm,0cm}}
  \subfloat[NewOrder TPM]
      { \includegraphics[scale=0.25] {figure/plot_tpcc_neworder_add_Warehouse_Warehouse_TPM_gather} \label{fig:new_order_add_terminal_gathered:tpm}}
  \captionsetup[subfigure]{margin={0cm,0cm}}
  \subfloat[NewOrder abort]
      { \includegraphics[scale=0.25]{figure/plot_tpcc_neworder_add_Warehouse_Warehouse_Abort_gather} \label{fig:new_order_add_terminal_gathered:abort}}
  \captionsetup[subfigure]{margin={0cm,0cm}}
\caption{throughput and abort rate of
...
\begin{figure}[htbp]
  ...
  \captionsetup[subfigure]{margin={0cm,0cm}}
  \subfloat[NewOrder TPM]
      { \includegraphics[scale=0.25] {figure/plot_tpcc_neworder_add_Warehouse_Warehouse_TPM_gather} \label{fig:new_order_add_warehouse_gathered:tpm}}
  \captionsetup[subfigure]{margin={0cm,0cm}}
  \subfloat[NewOrder abort]
