...
\begin{figure}[tbp]
    ...
    \subfloat[DP transaction, ${RM}$ knows nothing about other ${RM}$'s state when receiving \emph{Prepare}]
        { \includegraphics[scale=1] {figure/transaction_oneshot}  \label{fig:transaction_NDP}  }
    \captionsetup[subfigure]{oneside,margin={0.3cm,0cm}}
    \subfloat[NDP transaction, ${RM}$ would be told other ${RM}$'s state when receiving \emph{Prepare}]
...
A transaction has its ${in}$ value greater implies it has uncommitted dependent transactions and its ${RM}$ state not neither decided(``committed'' or    ``aborted'')
...
Early lock violation also fail to scale more than 200 terminas compare to the late lock violation.
...
We found that more idle resources allow the database use more agressive lock viiolation technique and deadlock detection to procesely avoid false positive abort.
