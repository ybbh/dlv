...
\begin{figure}[tbp]
  \centerline{\includegraphics[scale=0.8]{schedule_non_strict.pdf}}
  \caption{A non-strict but serializabile schedule ${H_2}$}
  \label{fig:non_strict_example}
\end{figure}

\begin{figure}[tbp]
  \centerline{\includegraphics[scale=0.8]{schedule_not_serializabile.pdf}}
  \caption{Schedule ${H_3}$, ${T_1}$ abort due to non-serializabile}
  \label{fig:schedule_abort_example}
\end{figure}

\begin{figure}[tbp]
  \centerline{\includegraphics[scale=0.8]{schedule_not_recoverable.pdf}}
...
%If any operation except the final complete one from $T_i$ is included in $H$ and ${T_i}$ would not abort deterministically,   this lock violation action is a late lock violation.
...
%If all the lock violation actions produced by a scheduler are late violation actions, and the scheduler is a late lock violation scheduler.
...
%But in safe lock violation time of the late violation, lock violation cannot lead to this cyclic dependencies.
...
%For early violation, the locking wait transactions and the lock violation transactions would contribute edges to WFG.
