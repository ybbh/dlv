...
%the following part is not understandable
...
When two-phase commit is adopted, there are several occasions to perform lock violation.
...
The correctness needs to be ensured by dependency tracing but not locking.
...
In this case, although we can ensure the serializability of a local schedule, we cannot guarantee the global serializability, because some ${RM}$s may abort deterministically. 
This is especially true for one-shot transactions.

\emph{DLV2}:
When a transaction has entered the second phase of 2PC and received the second round messages (i.e., the decision made by the ${TM}$ to commit the transaction), it allows other transactions to violate it locks.
In this case, deterministic abort is not possible before lock violation.

\emph{DLV1x}:
This is a variant of DLV1.
Before lock violation is enabled, ${TM}$ enforces a serializability test, in which it communicates with all ${RM}$.
If all ${RM}$ agree to commit, the test is passed, and lock violation is enabled.
This may require additional message round trip, especially for a one-shot transaction.
However, this message round trip can overlap with the 2PC and replication message flows.
More importantly, is a GDDB, as this message round trip is not between geo-replicated nodes, it is not expensive.
...
We define a transaction's \emph{safe violation time} as the time interval in which its locks can be violated without risking violating serializability.
As illustrated by Fig. \ref{fig:lock_violation_safe},
for early lock violation, there is no safe violation time, as there is no way to determine when lock violation can be safe, unless it performs a serializability test as DLV1 does.
...
Therefore, early lock violation needs to trace all ongoing dependencies to ensure serializability.
...
Second, early lock violation may face a much higher abort rate due to increased chance of serializability violation and cascade abort.
Section \ref{sec:deadlock_handling}
has shown a case of how early lock violation may lead to non-serializable schedules.
Regarding cascade abort, in late lock violation, only non-deterministic aborts can cause cascade abort, as no lock can be violated before a deterministic abort.
In early lock violation, all types of aborts can cause cascade abort.

Moreover, early lock violation may face worse overheads in dependency tracing. 
Whether late or early lock violation is adopted, we need to trace the commit dependencies among transactions to the commit order (or recoverability).
However, early violation needs to trace all \emph{rw, ww, wr} dependencies, while late lock violation only needs to trace \emph{wr} dependencies.
