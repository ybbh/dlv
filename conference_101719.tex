...
Speculative techniques, such as Controlled Lock Violation (CLV) and Early Lock Release (ELR), prove useful in shortening lock duration and boosting transaction processing performance.
...
On the contrary, lock violation at the second phase costs less on in-dependency tracing and cascade abort while sacrificing a certain amount of concurrency.
...
DLV can harness CLV's most benefit in GDDB, and it minimizes the cascade abort penalty and dependency tracing cost.

We organized the remainder of this paper as follows.
Section \ref{sec:relate_work} reviews related work.
In Section III, we explain why a strict schedule is unnecessary and may hurt the performance of distributed DBMS.
...
The deterministic scheduler enables active replication, which allows transactions to kick off synchronization at the earliest possible time.
...
For an interactive transaction, the $ TM $ has to explicitly notify the $RM$s they can prepare to commit.
...
Log order guarantees the recoverability of scheduling, which requires that a transaction cannot commit before the transactions it has commit dependencies on.
...
If for any operation except complete action not include deterministic abort(non-deterministic abort or commit) from $T_i$ is in $H$,   this lock violation action is late lock violation.
...
In contrast, \emph{late lock violation} is safe on serializability. If a transaction can only violate the locks of transactions that are ready to commit, it cannot result in cyclic data dependencies.
Early lock violation need maintains more dependencies to maintain safety. 
Second, early lock violation faces a much higher chance of abort, possible cascade abort. 
In late lock violation cases, only non-deterministic aborts can cause cascade abort as no lock can be violated before a deterministic abort. 
...
At this time, although the local schedule can enforce a serializable prefix, there may be another remote node that cannot guarantee serializability and other ${RM}$s abort deterministically, especially for one-shot transactions.
...
In this scenario, the lock violation scheduler has prevented deterministically abort for violating serializability before the violation.
...
Figure. \ref{fig:lock_violation_time} show the different timing of enabling lock violaion.
...
DLV1x releases the locks at the earliest possible time after determining that the transaction can have the most chance to commit. 
...
Late violation schedule, as it has promised commit projection,
it need only to ensure recoverability.
...
This anaomly is because $exp(H)$ performed the undo operations in the wrong order.
...
By only allowing a transaction to violate a lock with a larger transaction id, 
...
When using the deadlock detection algorithm,  blocking on locks is the most performance restriction if the DB is infrequently deadlocked.
...
To evaluate distributed transactions' scalability, we conducted experiments in both the gathered mode and the scattered mode.
...
We demonstrated that lock violation could be an effective measure to shorten the lock holding time and boost the performance of GDDB.
